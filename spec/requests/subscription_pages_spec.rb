...
describe "Subscription Pages" do
  ...
  describe "purchase page" do
  	...
  	describe "with swc_teacher plan as parameter" do
		...
		describe "with swc_lite plan as parameter" do
	  	before { visit purchase_path(id: "swc_lite") }
			let(:user) { FactoryGirl.create(:user) }
			...
			describe "when not signed in" do
		        ...
		        it { should have_selector('div.alert.alert-success', text: 'Thank you for purchasing!') }
		        it { should_not have_content('Your subscription will expire in 1 year.') }
						it { should have_field "Words" } # because its the new puzzle page.
				    it { should_not have_link "Buy Now!", href: plans_path }
		        ...
		        it { should have_selector('div.alert.alert-error') }
		        it { should have_selector('div.alert.alert-notify', text: "Credit Card Successfully Entered")}
					end
				end
